...
def calculate_core_params_monthly(month, installed_capacity):
    """修复：仅传月份和装机容量，从分月参数读取配置（避免参数不匹配）"""
...
def calculate_trade_power_arbitrage(month, total_trade_power, typical_df):
    ...
    if st.session_state.get("current_plant_type", "风电") == "光伏":
...
def export_annual_plan():
    ...
    scheme2_note = "套利曲线（两端转中午）" if plant_type == "光伏" else "直线曲线（24小时平均）"
    pv_config = get_pv_arbitrage_hours()["config"] if plant_type == "光伏" else {}
    ...
    for month in st.session_state.selected_months:
        ...
        summary_data.append({
            ...
            "电厂名称": st.session_state.get("current_power_plant", ""),
            "电厂类型": plant_type,
            "光伏核心时段": f"{pv_config.get('core_start', '')}-{pv_config.get('core_end', '')}点" if plant_type == "光伏" else "-",
            "光伏边缘时段": f"{pv_config.get('edge_start', '')}-{pv_config.get('edge_end', '')}点" if plant_type == "光伏" else "-",
    ...
    if st.session_state.current_plant_type == "光伏":
        ...
        with col_pv1:
            st.number_input(
                ...
                value=st.session_state["pv_core_start_key"], key="input_pv_core_start"
            )
        with col_pv2:
            st.number_input(
                "核心结束（点）", min_value=1, max_value=24,
                value=st.session_state["pv_core_end_key"], key="input_pv_core_end"
            )
        ...
        with col_pv3:
            st.number_input(
                ...
                value=st.session_state["pv_edge_start_key"], key="input_pv_edge_start"
            )
        with col_pv4:
            st.number_input(
                "边缘结束（点）", min_value=1, max_value=24,
                value=st.session_state["pv_edge_end_key"], key="input_pv_edge_end"
            )
    ...
    with st.expander("🔧 分月参数调整（单独修改）", expanded=False):
        ...
        with col_m2:
            ...
            mech_val = st.number_input(
                ...
                value=current_params["mechanism_value"], step=0.1, key=f"mech_val_{selected_month}"
            )
        ...
        with col_g2:
            ...
            gua_val = st.number_input(
                ...
                value=current_params["guaranteed_value"], step=0.1, key=f"gua_val_{selected_month}"
            )
        ...
        limit_rate = st.number_input(
            "限电率(%)", min_value=0.0, max_value=100.0,
            value=current_params["power_limit_rate"], step=0.1, key=f"limit_rate_{selected_month}"
        )
...
with col_import2:
    batch_file = st.file_uploader(
        "📥 批量导入Excel（含多月份子表）", type=["xlsx"], key="batch_import_file"
    )
...
with col_import3:
    st.session_state.selected_months = st.multiselect(
        "选择需要处理的月份", list(range(1, 13)), default=st.session_state.selected_months, key="month_multiselect"
    )
...
if st.session_state.selected_months and st.session_state.monthly_data:
    ...
    edit_month = st.selectbox(
        "选择要编辑的月份", st.session_state.selected_months, key="edit_month_select"
    )
...
if st.session_state.calculated and st.session_state.selected_months:
    ...
    st.subheader("1. 年度汇总")
    summary_data = []
    scheme2_note = "套利曲线" if plant_type == "光伏" else "直线曲线"
    for month in st.session_state.selected_months:
    ...
    view_month = st.selectbox(
        "选择查看的月份", st.session_state.selected_months, key="view_month_select"
    )
...
if st.session_state.calculated and st.session_state.selected_months:
    ...
    with col_adj1:
        adj_month = st.selectbox(
            "选择要调整的月份", st.session_state.selected_months, key="adj_month_select"
        )
    with col_adj2:
        adj_scheme = st.selectbox(
            "选择要调整的方案", ["方案一（典型曲线）", "方案二（套利/直线曲线）"], key="adj_scheme_select"
        )
