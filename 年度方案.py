...
# 新增：模式切换状态
if "mechanism_mode" not in st.session_state:
    st.session_state.mechanism_mode = "小时数"  # 机制电量模式：小时数/比例
if "guaranteed_mode" not in st.session_state:
    st.session_state.guaranteed_mode = "小时数"  # 保障性电量模式：小时数/比例
if "manual_market_hours" not in st.session_state:
    st.session_state.manual_market_hours = 0.0  # 手动输入的市场化小时数
if "auto_calculate" not in st.session_state:
    st.session_state.auto_calculate = True  # 是否自动计算市场化小时数
...
def calculate_market_hours(
    ...
    """
    自动计算市场化交易小时数
    :param gen_hours: 预估发电小时数
    :param power_limit_rate: 限电率(%)
    :param mechanism_value: 机制电量值（小时数/比例%）
    :param mechanism_mode: 机制电量模式（小时数/比例）
    :param guaranteed_value: 保障性电量值（小时数/比例%）
    :param guaranteed_mode: 保障性电量模式（小时数/比例）
    :return: 市场化交易小时数（≥0）
    """
    if gen_hours <= 0:
        return 0.0
    ...
    # 步骤1：扣减限电率
    limit_rate = power_limit_rate / 100  # 转换为小数
    available_hours = gen_hours * (1 - limit_rate)
    
    # 步骤2：扣减机制电量
    if mechanism_mode == "小时数":
        available_hours -= mechanism_value
    else:  # 比例模式
        mechanism_rate = mechanism_value / 100
        available_hours -= gen_hours * mechanism_rate
    
    # 步骤3：扣减保障性电量
    if guaranteed_mode == "小时数":
        available_hours -= guaranteed_value
    else:  # 比例模式
        guaranteed_rate = guaranteed_value / 100
        available_hours -= gen_hours * guaranteed_rate
...
# 区域/省份/月份/场站/装机容量
st.session_state.current_region = st.sidebar.selectbox("选择区域", list(REGIONS.keys()),
    index=list(REGIONS.keys()).index(st.session_state.current_region), key="region_select")
...
st.session_state.current_province = st.sidebar.selectbox("选择省份/地区", current_province_list,
    index=current_province_list.index(st.session_state.current_province), key="province_select")
st.session_state.current_month = st.sidebar.selectbox("选择月份", MONTHS,
    index=st.session_state.current_month-1, key="month_select")
st.session_state.current_site = st.sidebar.text_input("场站名称", value=st.session_state.current_site,
    key="site_name_input", placeholder="如：张家口风电场")
installed_capacity = st.sidebar.number_input("装机容量(MW)", min_value=0.0, value=0.0, step=0.1,
    key="installed_capacity", help="场站总装机容量，单位：兆瓦")
...
with col_mech1:
    st.session_state.mechanism_mode = st.selectbox(
        "输入模式", ["小时数", "比例(%)"], 
...
with col_mech2:
    ...
    mechanism_value = st.number_input(
        ...
        key="mechanism_value", help=f"机制电量{st.session_state.mechanism_mode}"
    )
...
with col_gua1:
    st.session_state.guaranteed_mode = st.selectbox(
        "输入模式", ["小时数", "比例(%)"], 
...
with col_gua2:
    ...
    guaranteed_value = st.number_input(
        ...
        key="guaranteed_value", help=f"保障性电量{st.session_state.guaranteed_mode}"
    )

# 限电率（固定为百分比）
power_limit_rate = st.sidebar.number_input(
    "限电率(%)", min_value=0.0, max_value=100.0, value=0.0, step=0.1,
    key="power_limit_rate", help="场站当月限电比例，0-100%"
)

# 市场化交易小时数（自动/手动切换）
st.sidebar.write("#### 市场化交易小时数")
st.session_state.auto_calculate = st.sidebar.toggle(
    "自动计算", value=st.session_state.auto_calculate,
    key="auto_calculate_toggle", help="勾选：按公式自动计算；取消：手动输入"
)
if st.session_state.auto_calculate:
    ...
    st.sidebar.number_input(
        ...
        key="market_hours_auto"
    )
else:
    # 手动输入模式
    market_hours = st.sidebar.number_input(
        "手动输入", min_value=0.0, value=st.session_state.manual_market_hours, step=0.1,
        key="market_hours_manual"
    )
...
with col2:
    import_btn = st.file_uploader("📤 导入数据(CSV/Excel)", type=["csv", "xlsx"], key="data_import")
with col3:
...
if load_btn:
    if not st.session_state.current_province or not st.session_state.current_month or not st.session_state.current_site:
        ...
        loaded_data = load_data_from_file(st.session_state.current_province, st.session_state.current_month, st.session_state.current_site)
...
if generate_plan_btn:
    try:
        ...
        with overview_col4:
            st.metric("分配误差(MWh)", f"{round(sum(trade_power_df['市场化交易电量(MWh)']) - total_trade_power, 4):.4f}")
        ...
        # 可视化
        st.subheader("时段比重分布")
        st.bar_chart(trade_power_df.set_index("时段")["时段比重(%)"], use_container_width=True, y_label="比重(%)")
