...
REDUNDANT_KEYWORDS = [
    ...
    "ylxxhfd", "yxxchfd", "依兰县协合风力发电有限公司"  # 新增专属冗余词
]
# 扩展科目编码-名称映射（兼容新科目，自动适配所有场站）
TRADE_CODE_MAP = {
    # 原有科目
    "0101010101": "优先发电交易",
...
DATA_RULES = {
    "电量(兆瓦时)": {"min": -1000, "max": 5000},  # 支持负数电量
    "电价(元/兆瓦时)": {"min": 0, "max": 2000},
    "电费(元)": {"min": -10000, "max": 10000000}  # 支持负数费用
}
# 双场站拆分关键词
...
def split_double_station_tables(all_tables, pdf_text):
    ...
    for row in merged_rows:
        ...
        if any(keyword in row_str for keyword in STATION_SPLIT_KEYWORDS):
            # 保存上一段（非空）
            if current_segment:
                station_segments.append((current_station, current_segment))
            # 提取当前场站名称（从“机组”行）
            for cell in row:
...
def get_trade_name(trade_code, trade_text):
    ...
    for key, name in TRADE_KEYWORDS.items():
        if key in trade_text:
            return name
    # 未匹配到则返回清理后的文本（兼容新科目）
    return remove_redundant_text(trade_text) if trade_text else "未识别科目"
...
def parse_single_station_data(station_name, table_segment, company_name, clear_date):
    ...
    for row_idx in range(data_start_idx, len(valid_rows)):
        ...
        # 保留有数据的科目
        if quantity is None and fee is None and "未识别科目" not in trade_name:
            continue
...
# ---------------------- Streamlit应用（列展示/下载差异化） ----------------------
def main():
    st.set_page_config(page_title="通用日清分数据提取工具（双场站版）", layout="wide")
    
    st.title("📊 通用现货日清分结算单数据提取工具（双场站拆分版）")
    st.markdown("**核心特性：双场站自动拆分 | 全科目兼容 | 列展示/下载差异化 | 支持负数数据**")
    st.divider()
    ...
    if uploaded_files and st.button("🚀 开始批量提取", type="primary"):
        ...
        # 显示结果（高亮小计行和双场站）
        st.subheader("📈 批量提取结果（双场站已自动拆分）")
        def highlight_rows(row):
            if row["科目名称"] == "当日小计":
                return ["background-color: #e6f3ff"] * len(row)
            elif "双发A风电场" in row["场站名称"]:
                return ["background-color: #f0fff4"] * len(row)
            elif "双发B风电场" in row["场站名称"]:
        ...
        st.success("✅ 批量提取完成！双场站已自动拆分，平台展示原始编码/文本，Excel下载仅保留核心列")
    
    else:
        st.info("👆 请上传单/双场站的现货日清分结算单PDF（支持多页面）")
